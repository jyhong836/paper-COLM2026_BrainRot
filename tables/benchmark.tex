...
% \centering
% \scriptsize
% \caption{Evaluating \llama (Base) after being trained on varying mixtures of junk and control data. Colors indicate the \mycolorbox{myred!80}{worse}/\mycolorbox{royalblue!60}{better} performance than the base model in the row. 
%        All scores range from $0$ to $100$. For RULER, we select a subset of tasks to present, and the full results are in \cref{tab:benchmark_llama_full}. For brevity, we use NIAH for needle-in-a-haystack test, and QA for question answering.
%        % ASR for attack success rate. 
%        % \junyuan{Highlight the DARK personalities.}
%        }
